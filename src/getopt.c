...
static struct gop_option* opt_search(struct gop_ctx* ctx, int optchar) {
  int i = 0;
  for (struct gop_option* o=ctx->longopts; o->name; ++o, ++i) {
...
static int single(struct gop_ctx* ctx) {
  int optchar = -1;
  struct gop_option* optdecl = NULL;
  int argc = ctx->argc;
...
int gop_next(/*int argc, char* const argv[], 
  ...
  struct gopoption const* match = NULL;
  int num_matches = 0;
  ...
  if (num_matches == 1) {
    ...
    ctx->longindex = (int*)(match - longopts);
